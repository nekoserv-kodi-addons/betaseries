...
betaseries_host = 'https://api.betaseries.com'
...
def search_episode_url(search_params):
    ...
    episode_url = '%s/episodes/search?show_id=%s&number=S%#02dE%#02d&key=%s&v=%s' % (
        betaseries_host, show_id, int(search_params['season']), int(search_params['episode']),
        SettingService.get(SettingEnum.BETASERIES_APIKEY), betaseries_api_version)
    return episode_url


def get_show_id_from_betaseries(imdb_id):
    # get betaseries show id from imdb_id
    show_url = '%s/shows/display?imdb_id=%s&key=%s&v=%s' % (betaseries_host,
                                                            imdb_id,
...
def get_subtitles_from_betaseries(episode_id):
    ...
    list_url = '%s/subtitles/episode?id=%s&key=%s&v=%s' % (
        betaseries_host, episode_id, SettingService.get(SettingEnum.BETASERIES_APIKEY),
        betaseries_api_version)
    try:
...
def generate_betaseries_url(search_params, filename):
    if search_params['mode'] == 'tvshow':
        ...
        episode_url = '%s/episodes/scraper?file=%s&key=%s&v=%s' % (
            betaseries_host, quote(filename), SettingService.get(SettingEnum.BETASERIES_APIKEY),
            betaseries_api_version)
    return episode_url
