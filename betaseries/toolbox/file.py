...
def extract_file(temp_file, filename):
    tmpdir = SettingService.get(SettingEnum.TEMP_DIR)
    init_nb_files = len(listdir(tmpdir))
    log('number of files : %s' % init_nb_files)
    ...
    while (count_files_in_dir(tmpdir) == init_nb_files) and (wait_time < time_threshold):  # nothing yet extracted
